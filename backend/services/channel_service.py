...
def edit_channel_service(channel_id: int, data: Channels_input, authorization: str, db: Session):
    ...
    if not is_owner and not is_member:
        raise HTTPException(status_code=403, detail="You must be a member of this organization to edit channels")
    
    if channel.type == "general" and not is_owner:
        raise HTTPException(status_code=403, detail="Only the organization owner can edit general channels")
...
def delete_channel_service(channel_id: int, authorization: str, db: Session):
    ...
    if not is_owner and not is_member:
        raise HTTPException(status_code=403, detail="You must be a member of this organization to delete channels")
    
    if channel.type == "general" and not is_owner:
        raise HTTPException(status_code=403, detail="Only the organization owner can delete general channels")
