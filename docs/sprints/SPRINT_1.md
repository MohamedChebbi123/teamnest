...
| ------ | ------------ | -------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| US-1.1 | Onboarding   | High     | As a **visitor**, I want to browse the landing page, so that I can learn what TeamNest offers. | **T-1.1.1** Design landing layout (hero, features, footer)<br>**T-1.1.2** Implement responsive Next.js components<br>**T-1.1.3** Wire CTAs to signup / login |
| US-1.2 | Registration | High     | As a **visitor**, I want to register, so that I can create an account.                         | **T-1.2.1** Build registration form UI<br>**T-1.2.2** `POST /auth/register` endpoint<br>**T-1.2.3** Hash password (bcrypt) and persist user                  |

### User

| ID      | Epic                | Priority | Story                                                                                              | Subtasks                                                                                                                       |
| ------- | ------------------- | -------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| US-2.1  | Email Verification  | High     | As a **user**, I want to verify my email, so that my account is activated.                         | **T-2.1.1** Generate and store 6-digit code<br>**T-2.1.2** `POST /auth/verify-email` endpoint<br>**T-2.1.3** Flip `is_verified` on success                |
| US-2.2  | Email Verification  | High     | As a **user**, I want to resend the verification code, so that I'm not blocked.                    | **T-2.2.1** `POST /auth/resend-code` with rate-limit<br>**T-2.2.2** Wire "Resend" button on verification screen                                            |
| US-2.3  | Session Management  | High     | As a **user**, I want to stay signed in, so that I don't log in every visit.                       | **T-2.3.1** Issue JWT access + refresh tokens<br>**T-2.3.2** Refresh-token rotation endpoint<br>**T-2.3.3** Persist sessions table                        |
| US-2.4  | Session Management  | High     | As a **user**, I want to log out from one or all devices, so that I can secure my account.         | **T-2.4.1** `POST /auth/logout` (current)<br>**T-2.4.2** `POST /auth/logout-all`<br>**T-2.4.3** Revoke matching refresh tokens                            |
| US-2.5  | Password Management | High     | As a **user**, I want to reset my password by email, so that I can recover access.                 | **T-2.5.1** `POST /auth/forgot-password` sends code via Resend<br>**T-2.5.2** `POST /auth/reset-password` verifies + updates<br>**T-2.5.3** Reset UI flow |
| US-2.6  | Password Management | Medium   | As a **user**, I want to change my password, so that I can rotate it.                              | **T-2.6.1** `POST /auth/change-password` with old-password check<br>**T-2.6.2** Re-hash and persist new password                                          |
| US-2.7  | Profile Management  | Medium   | As a **user**, I want to edit my profile (avatar, name, country, phone), so that it stays current. | **T-2.7.1** `PATCH /users/me`<br>**T-2.7.2** Avatar upload via Cloudinary<br>**T-2.7.3** Profile-edit form UI                                             |
| US-2.8  | Profile Management  | Low      | As a **user**, I want to set my presence, so that others know my availability.                     | **T-2.8.1** Add `presence` enum field on user<br>**T-2.8.2** WebSocket broadcast on change                                                                 |
| US-2.9  | Preferences         | Low      | As a **user**, I want light/dark theme, so that the look matches my preference.                    | **T-2.9.1** Add theme toggle component<br>**T-2.9.2** Persist preference (localStorage + user setting)                                                    |
| US-2.10 | Onboarding          | Low      | As a **user**, I want a guided tour, so that I learn the basics quickly.                           | **T-2.10.1** Integrate tour library (Driver.js / Shepherd)<br>**T-2.10.2** Define onboarding step list                                                    |
