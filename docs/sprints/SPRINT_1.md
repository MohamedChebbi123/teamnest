...
| -------------------- | ------ | -------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Visitor Onboarding   | US-1.1 | High     | As a **visitor**, I want to browse the landing page, so that I can learn what TeamNest offers. | **T-1.1.1** Design landing layout (hero, features, footer)<br>**T-1.1.2** Implement responsive Next.js components<br>**T-1.1.3** Wire CTAs to signup / login |
| Account Registration | US-1.2 | High     | As a **visitor**, I want to register, so that I can create an account.                         | **T-1.2.1** Build registration form UI<br>**T-1.2.2** `POST /auth/register` endpoint<br>**T-1.2.3** Hash password (bcrypt) and persist user                  |
...
| Login Sessions          | US-2.3  | High     | As a **user**, I want to stay signed in, so that I don't log in every visit.                       | **T-2.3.1** Issue JWT access + refresh tokens<br>**T-2.3.2** Refresh-token rotation endpoint<br>**T-2.3.3** Persist sessions table                        |
...
| Password Reset & Change | US-2.5  | High     | As a **user**, I want to reset my password by email, so that I can recover access.                 | **T-2.5.1** `POST /auth/forgot-password` sends code via Resend<br>**T-2.5.2** `POST /auth/reset-password` verifies + updates<br>**T-2.5.3** Reset UI flow |
...
| Profile Editing         | US-2.7  | Medium   | As a **user**, I want to edit my profile (avatar, name, country, phone), so that it stays current. | **T-2.7.1** `PATCH /users/me`<br>**T-2.7.2** Avatar upload via Cloudinary<br>**T-2.7.3** Profile-edit form UI                                             |
...
| UI Preferences          | US-2.9  | Low      | As a **user**, I want light/dark theme, so that the look matches my preference.                    | **T-2.9.1** Add theme toggle component<br>**T-2.9.2** Persist preference (localStorage + user setting)                                                    |
| Guided Tour             | US-2.10 | Low      | As a **user**, I want a guided tour, so that I learn the basics quickly.                           | **T-2.10.1** Integrate tour library (Driver.js / Shepherd)<br>**T-2.10.2** Define onboarding step list                                                    |
