...
| --------------------- | ------- | -------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| AI Assistant (RAG)    | US-9.1  | Medium   | As a **member**, I want to ask the AI about my org, so that I get quick answers.                                     | **T-9.1.1** `POST /assistant/ask` endpoint<br>**T-9.1.2** Retrieve context from Pinecone<br>**T-9.1.3** Build prompt + call Groq LLM (llama-3.3-70b) |
|                       | US-9.2  | Low      | As a **member**, I want the AI to use our uploaded documents, so that answers come from our files.                   | **T-9.2.1** Document chunking via LlamaIndex<br>**T-9.2.2** Embed chunks + store in Pinecone with org namespace                                      |
|                       | US-9.3  | Low      | As a **member**, I want to open a PDF inline and ask the AI about it, so that I can extract answers from long files. | **T-9.3.1** Inline PDF viewer with selectable page range<br>**T-9.3.2** Pass active document scope into RAG context                                  |
| Global Message Search | US-10.1 | High     | As a **member**, I want to search across org messages, so that I find info fast.                                     | **T-10.1.1** Vector similarity search endpoint over Pinecone<br>**T-10.1.2** Global search bar UI in header<br>**T-10.1.3** Hydrate results from DB  |

### Org Owner — Billing, Logs & Undo

| Epic               | ID      | Priority | Story                                                                                                 | Subtasks                                                                                                                                                |
| ------------------ | ------- | -------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pro Plan Billing   | US-12.2 | High     | As an **org owner**, I want to subscribe to the Pro plan, so that I can unlock plan limits.           | **T-12.2.1** Stripe Checkout session endpoint<br>**T-12.2.2** Stripe webhook updates org subscription<br>**T-12.2.3** Plan-selection UI on billing page |
|                    | US-12.3 | High     | As an **org owner**, I want to cancel the Pro subscription, so that I can downgrade.                  | **T-12.3.1** Call Stripe `subscriptions.cancel`<br>**T-12.3.2** Webhook downgrade flow resets plan limits                                               |
| Audit Activity Log | US-12.4 | Medium   | As an **owner or admin**, I want to view the activity log, so that I have an audit trail.             | **T-12.4.1** `activity_log` table + `GET /activity-log` endpoint<br>**T-12.4.2** Filterable log UI (actor, action, date range)                          |
|                    | US-12.5 | Low      | As an **org owner**, I want to undo a reversible logged action, so that I can recover from a mistake. | **T-12.5.1** Per-action reversal handlers (with safety guard)<br>**T-12.5.2** Undo button on eligible log entries                                       |
