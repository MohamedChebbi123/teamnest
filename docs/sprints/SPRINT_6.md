...
| Global Search      | US-10.1 | High    | As a **member**, I want to search across org messages, so that I find info fast.                                     | **T-10.1.1** Vector similarity search endpoint over Pinecone<br>**T-10.1.2** Global search bar UI in header<br>**T-10.1.3** Hydrate results from DB    |

### Org Owner — Billing, Logs & Undo

| Epic           | ID      | Priority | Story                                                                                                  | Subtasks                                                                                                                  |
| -------------- | ------- | -------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| Billing        | US-12.2 | High     | As an **org owner**, I want to subscribe to the Pro plan, so that I can unlock plan limits.            | **T-12.2.1** Stripe Checkout session endpoint<br>**T-12.2.2** Stripe webhook updates org subscription<br>**T-12.2.3** Plan-selection UI on billing page |
...
| Activity Log   | US-12.4 | Medium   | As an **owner or admin**, I want to view the activity log, so that I have an audit trail.              | **T-12.4.1** `activity_log` table + `GET /activity-log` endpoint<br>**T-12.4.2** Filterable log UI (actor, action, date range)                          |
