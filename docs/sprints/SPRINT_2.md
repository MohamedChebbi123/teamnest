...
| --------------------- | ------ | -------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Organization Creation | US-6.1 | High     | As a **member**, I want to create an organization, so that I can host my workspace. | **T-6.1.1** Org creation form UI<br>**T-6.1.2** `POST /organizations` endpoint<br>**T-6.1.3** Optional logo upload via Cloudinary |
| Member Invitations    | US-6.2 | High     | As a **member**, I want to join an org with an invite, so that I can collaborate.   | **T-6.2.1** Invite-acceptance endpoint (token → membership)<br>**T-6.2.2** Join-via-invite-link landing UI                        |
| Member Directory      | US-6.3 | High     | As a **member**, I want to see all org members, so that I have an overview.         | **T-6.3.1** `GET /organizations/{id}/members` with pagination<br>**T-6.3.2** Members list UI with search filter                   |
...
| Epic                  | ID      | Priority | Story                                                                                         | Subtasks                                                                                                                                            |
| --------------------- | ------- | -------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Member Invitations    | US-11.1 | High     | As an **org admin**, I want to invite members by email, so that they can join.                | **T-11.1.1** `POST /invitations` generates invite token<br>**T-11.1.2** Send invite email via Resend<br>**T-11.1.3** Invite form UI (single + bulk) |
...
| Organization Creation | US-11.3 | Medium   | As an **org admin**, I want to update the organization, so that I can keep it accurate.       | **T-11.3.1** `PATCH /organizations/{id}` for metadata<br>**T-11.3.2** Org-settings form UI                                                          |
| Team Creation         | US-11.4 | High     | As an **org admin**, I want to create teams, so that I can group members by project.          | **T-11.4.1** `POST /teams` scoped to organization<br>**T-11.4.2** Team-creation modal with lead selector                                            |

### Org Owner

| Epic                  | ID      | Priority | Story                                                                                 | Subtasks                                                                                                               |
| --------------------- | ------- | -------- | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Organization Deletion | US-12.1 | Medium   | As an **org owner**, I want to delete my organization, so that I can decommission it. | **T-12.1.1** `DELETE /organizations/{id}` (owner-only)<br>**T-12.1.2** Cascade cleanup of teams, channels, memberships |

### Team Lead

| Epic                   | ID      | Priority | Story                                                                                                     | Subtasks                                                                                                                     |
| ---------------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Team Settings          | US-13.1 | Medium   | As a **team lead**, I want to update or delete my team, so that I can keep it accurate or wind it down.   | **T-13.1.1** `PATCH /teams/{id}` and `DELETE /teams/{id}`<br>**T-13.1.2** Team-settings page with destructive-action confirm |
| Team Member Management | US-13.2 | High     | As a **team lead**, I want to add members to my team, so that they get access.                            | **T-13.2.1** `POST /teams/{id}/members` endpoint<br>**T-13.2.2** Member-picker UI sourcing org directory                     |
...
| Epic             | ID      | Priority | Story                                                                                 | Subtasks                                                                                                |
| ---------------- | ------- | -------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Member Directory | US-15.1 | Low      | As a **team member**, I want to view a teammate's profile, so that I know their role. | **T-15.1.1** `GET /users/{id}` returning public profile fields<br>**T-15.1.2** Teammate profile page UI |
