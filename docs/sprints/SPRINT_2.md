...
| US-6.4 | Directory             | High     | As a **member**, I want to see the teams in my org, so that I can navigate to one.  | **T-6.4.1** `GET /organizations/{id}/teams` endpoint<br>**T-6.4.2** Teams grid/list UI with navigation                             |
...
| US-11.2 | Membership            | High     | As an **org admin**, I want to accept or reject join requests, so that I control who gets in.  | **T-11.2.1** `GET /organizations/{id}/join-requests`<br>**T-11.2.2** Accept/reject endpoints<br>**T-11.2.3** Admin review UI with bulk actions        |
...
| US-13.3 | Team Membership    | Medium   | As a **team lead**, I want to grant or revoke a member's permissions, so that responsibilities are clear. | **T-13.3.1** Permission/role field on team membership<br>**T-13.3.2** Role-update endpoint with lead-only guard              |
| US-13.4 | Team Membership    | Medium   | As a **team lead**, I want to kick a member, so that I can remove unwanted people.                        | **T-13.4.1** `DELETE /teams/{id}/members/{userId}`<br>**T-13.4.2** Confirmation dialog before removal                        |
